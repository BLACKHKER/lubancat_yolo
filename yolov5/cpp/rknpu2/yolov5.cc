...
int init_yolov5_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    ...
    // init RKNN with extended parameters for performance
    rknn_init_extend extend_param;
    memset(&extend_param, 0, sizeof(rknn_init_extend));

    ret = rknn_init(&ctx, model, model_len, RKNN_FLAG_PRIOR_MEDIUM | RKNN_FLAG_COLLECT_PERF_MASK, &extend_param);
    free(model);
    if (ret < 0)
    {
